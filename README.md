...
- **反向代理**: Caddy 提供自動 HTTPS、HTTP/2、負載均衡和 WebSocket 代理
- **容器化部署**: Docker Compose 編排多服務架構，一鍵啟動全部服務
...
專案提供完整的 Docker Compose 配置,一鍵啟動所有服務。
...
1. **deps** - 安裝依賴 (`npm ci`)
2. **builder** - 構建應用 (Prisma 生成 + Next.js 構建)
3. **runner** - 生產運行 (Alpine Linux 最小化鏡像)

**特點**:
- ✅ 基於 `node:20-alpine` 最小化鏡像
- ✅ 多階段構建減少最終鏡像大小
- ✅ 包含 OCPP Server 完整依賴
- ✅ 支援 Next.js Standalone 輸出
- ✅ 非 root 用戶運行提高安全性
